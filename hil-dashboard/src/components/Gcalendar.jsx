...
const Gcalendar = () => {
  ...
  const handleClick = (info) =>{
    if(params.name){
      ...
      toast.error('Please select a HiL')
    }
  ...
  const handleEmptyName = () => toast.error('Name filed is required.')
  
  const handleEmptyNotes = () => toast.error('Name Notes is required.')
  const handleSubmit = () =>{
          ...
          <ul>
              {
                post.map((items, index)=>(
                  <li key={index}><Link to={`/gencalendar/${items.name}?message=${items.name}`}
